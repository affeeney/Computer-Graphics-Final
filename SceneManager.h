...
public:
	...
	struct Light
	{
		...
		glm::vec3 position;
		glm::vec3 direction; // Used for spotlights
		glm::vec3 color;
		float intensity;
		float cutOff; // Used for spotlights
		float outerCutOff; // Used for spotlights
		float constant;
		float linear;
		float quadratic;
		bool bActive;
...
private:
	...
	std::vector<Light> m_lights; // Store multiple lights
	...
	// Set light properties into the shader
